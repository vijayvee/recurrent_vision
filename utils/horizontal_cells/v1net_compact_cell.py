...
class V1NetCompact(rnn_cell_impl.RNNCell):
  ...
  def __init__(self,
    ...
    """
    super(V1NetCompact, self).__init__(name=name)
  ...
  def call(self, inputs, state, scope=None):
    ...
    forget_gate, output_gate = tf.split(conv_xh,
                          num_or_size_splits=2,
                          axis=-1,
                          )
    hidden_hor = (conv_exc, conv_shunt)
    
    new_input = _horizontal(inputs, hidden_hor, self.control)
    new_cell = tf.nn.sigmoid(forget_gate) * cell
    new_cell += (1 - tf.nn.sigmoid(forget_gate)) * tf.nn.tanh(new_input)
